...
class Player:
    ...
    def go(self, _x, _y):
        if self.alive:
            if abs(self.x - _x) + abs(self.y - _y) == 1 and 1 <= _x <= num_rows and 1 <= _y <= num_cols:
...
def get_next_move(t):
    ...
    for i in range(2):
        ...
        output_path = f'{root}/{player_sub}/{players_id[i]}/MOVE.OUT'
        open(output_path, 'w').close()
        try:
            subprocess.check_call([f'{players_id[i]}.exe'], timeout=1, cwd=f'{root}/{player_sub}/{players_id[i]}', shell=True, **subprocess_args(False))
        except Exception as e:
            logger.append(str(e))
            PROC_NAME = f"{players_id[i]}.exe"
